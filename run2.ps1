...
if (!($version -and $version -match $match_v)) {
    if ($old_os) { 
        ...
        # For latest version, use generic installer
        $use_generic = $true
        # Set a high version for comparison to trigger upgrade if needed
        $onlineFull = "1.2.99.999.g00000000-0000"
    }
}
else {
    if ($old_os) {
...
function downloadSp() {
    ...
    if ($use_generic) {
        $web_Url = "https://download.scdn.co/SpotifySetup.exe"
...
if (-not $spotifyInstalled -or $upgrade_client) {
    ...
    # Set $online to match installed for patch checks
    $online = ($offline -split '\.' | Select-Object -First 4) -join '.'
